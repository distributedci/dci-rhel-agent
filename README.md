...
This file has the credential associated to the SUT (also kwnown as a `RemoteCI` in the [DCI web dashboard](https://www.distributed-ci.io). The partner team administrator has to create a Remote CI in the DCI web dashboard, copy the relative credential and paste it locally on the Jumpbox to `/etc/dci-rhel-agent/dcirc.sh`.
...
| beaker_lab.jumpbox_fqdn       | False    | FQDN           | FQDN of DCI Jumpbox jumpbox.                                                                                                        |
