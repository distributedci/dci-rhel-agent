...
from ansible.module_utils.productmd import common
from ansible.module_utils.productmd import composeinfo
...
class TreeInfo(common.MetadataBase):
    def __init__(self):
        super(common.MetadataBase, self)
    ...
    def _get_parser(self):
        return common.SortedConfigParser()

    def parse_file(self, f):
        # parse file, return parser or dict with data
        f.seek(0)
        parser = common.SortedConfigParser()
        parser.read_file(f)
        return parser
    ...
    def dump(self, f, main_variant=None):
        ...
        """
        self.validate()
        with common.open_file_obj(f, "w") as f:
...
class Header(common.Header):
    ...
    def serialize(self, parser):
        ...
        parser.set(self._section, "version", ".".join([str(i) for i in common.VERSION]))
...
class BaseProduct(common.MetadataBase):
    """
...
class Release(BaseProduct):
    ...
    def major_version(self):
        ...
        if self.version is None:
            return None
        return common.get_major_version(self.version)
    ...
    def minor_version(self):
        ...
        if self.version is None:
            return None
        return common.get_minor_version(self.version)


# Note: [tree]/variants is read/written in the Variants class
class Tree(common.MetadataBase):
...
class Variants(composeinfo.VariantBase):
    def __init__(self, metadata):
...
class VariantPaths(common.MetadataBase):
    """
...
class Variant(composeinfo.VariantBase):
    def __init__(self, metadata):
...
class Images(common.MetadataBase):
...
class Stage2(common.MetadataBase):
    def __init__(self, metadata):
...
class Checksums(common.MetadataBase):
...
class Media(common.MetadataBase):
...
class General(common.MetadataBase):
